angular.module('app').controller('roomCtrl', 
...
function($scope, $http, uiGmapGoogleMapApi, MatchSvc, AddRoomies, $q, $timeout, UpdateSvc, QuerySvc, $sessionStorage)
	{
		...
		$scope.addRoom = function()
		{
			...
			else
			{
				...
				{
					console.log('In addRoomies');
					defer.resolve(AddRoomies($scope.roomies))
					additionalID = defer.promise;
					console.log('In addRoomies additionalID: ' + JSON.stringify(additionalID));
					console.log('Will return from addRoomies');
					return additionalID;
				}
				var postRoomAndUpdate = function()
				{
					console.log('In postRoomAndUpdate');
					$scope.room.roomie = additionalID;
					console.log('In postRoomAndUpdate additionalID: ' + JSON.stringify(additionalID));
					$http.post('/room', $scope.room)
					.then(function(response)
					{
						console.log('the room posted: ');
						...
						// send mail to the annexed roomies
					});
				};
